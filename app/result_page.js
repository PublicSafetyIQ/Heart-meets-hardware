  ...
  async function handleEmail() {
    ...
    try {
      await fetch('/api/send-email', {
        ...
        body: JSON.stringify({ email, primary, secondary, blend, scores }),
      });
      setSent(true);
    } catch (e) {
      console.error(e);
    ...
    <main className="min-h-screen flex flex-col px-6 py-10 max-w-md mx-auto">
      ...
      <div className="bg-hmh-gray rounded-lg p-4 mb-8">
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          {[
            { label: 'D', score: scores.D, color: '#B91C1C' },
            { label: 'I', score: scores.I, color: '#C9A23E' },
            { label: 'S', score: scores.S, color: '#1E3A8A' },
            { label: 'C', score: scores.C, color: '#F5EDD6' },
      ...
      <div className="bg-hmh-gray rounded-lg p-5 mb-6">
        ...
        <p className="text-hmh-cream-dim text-xs mb-4">
          One email with your full result and links to learn more about your style.
        </p>
        {sent ? (
...
function TypeBadge({ letter, color, size }) {
  ...
  return (
    <div
      className={`${dim} rounded-full flex items-center justify-center font-serif font-bold border-2`}
  ...
  return (
    <Suspense fallback={
      <main className="min-h-screen flex items-center justify-center">
        <p className="text-hmh-cream-dim">Loading your results...</p>
      </main>
    }>
      <ResultContent />
    </Suspense>
