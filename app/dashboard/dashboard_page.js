...
function DiscWheel({ data }) {
  ...
  // Correct DiSC layout: D=top-left, I=top-right, S=bottom-right, C=bottom-left
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-full max-w-xs mx-auto mb-6" style={{aspectRatio:'1'}}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          {/* D = top-left quadrant (9 o'clock sweeping clockwise to 12) */}
          <path d="M50,50 L2,50 A48,48 0 0,1 50,2 Z" fill="#B91C1C" opacity="0.25"/>
          {/* I = top-right quadrant (12 o'clock sweeping clockwise to 3) */}
          <path d="M50,50 L50,2 A48,48 0 0,1 98,50 Z" fill="#C9A23E" opacity="0.25"/>
          {/* S = bottom-right quadrant (3 o'clock sweeping clockwise to 6) */}
          <path d="M50,50 L98,50 A48,48 0 0,1 50,98 Z" fill="#1E3A8A" opacity="0.3"/>
          {/* C = bottom-left quadrant (6 o'clock sweeping clockwise to 9) */}
          ...
          {/* Dots first */}
          {dots.map(d => (
            <circle key={d.key} cx={d.cx} cy={d.cy} r="2.8"
              fill={d.fill} opacity="0.9" stroke="#000" strokeWidth="0.4"/>
          ))}
          {/* Labels on top with dark background — D top-left, I top-right, S bottom-right, C bottom-left */}
