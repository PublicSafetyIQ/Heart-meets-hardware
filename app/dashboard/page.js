...
function PieChart({ slices, size = 130 }) {
  ...
  slices.forEach(({ value, color }, i) => {
    ...
    let pathD;
    if (value === total) {
      // Full circle — SVG arc can't go from a point to itself, use two semicircles
      ...
      angle = angle; // already incremented above
    }

    // For multi-slice, advance angle was done inside else; for 100% case reset mid
    const labelAngle = value === total ? -Math.PI / 2 : mid;
    const lr = R * 0.68;
    ...
    <div className="grid grid-cols-2 gap-3">
      {POLL_QUESTIONS.map(q => {
        ...
        const slices = q.options.map(opt => ({
          label: opt, value: tally[opt] || 0, color: OPTION_COLORS[opt] || '#888',
        }));
            ...
            <div className="mt-3 w-full space-y-1">
              {q.options.map(opt => (
                <div key={opt} className="flex items-center gap-2 text-xs" style={{color:'#ccc'}}>
                  ...
                  <span>{opt}</span>
                  <span className="ml-auto" style={{color:'#888'}}>{tally[opt] || 0}</span>
                </div>
              ))}
            </div>
            <p className="mt-2 text-xs" style={{color:'#555'}}>{total} response{total !== 1 ? 's' : ''}</p>
          </div>
        );
      })}
    </div>
