...
const STOP_WORDS = new Set(['the','and','a','an','to','of','in','is','it','for','on','at','i','my','we','our','with','are','be','this','that','have','not','from','but','or','as','by','so','do','they','their','what','when','how','all','no','can','if','just','more','get','has','was','been','will','need','its','very','too','also','about','up','out','like','time','right','know','think','really','because','feel','work','using','make','dont']);
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    supabase.from('poll_responses').select('*').in('poll_id',['poll_1','poll_2','poll_3','poll_4']).then(({ data }) => {
      if (!data) return;
      const agg = { poll_1:{}, poll_2:{}, poll_3:{}, poll_4:{} };
      data.forEach(({ poll_id, answer }) => { if (agg[poll_id]) agg[poll_id][answer] = (agg[poll_id][answer]||0)+1; });
      setPollData(agg);
    });
    ...
    const discCh = supabase.channel('disc-live')
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'disc_responses'},(p)=>setResponses((prev)=>[...prev,p.new]))
      .subscribe();

    const pollCh = supabase.channel('poll-live')
      .on('postgres_changes',{event:'INSERT',schema:'public',table:'poll_responses'},(p)=>{
        const { poll_id, answer } = p.new;
        if (['poll_1','poll_2','poll_3','poll_4'].includes(poll_id)) {
          setPollData((prev)=>({...prev,[poll_id]:{...prev[poll_id],[answer]:(prev[poll_id][answer]||0)+1}}));
        }
        if (poll_id==='brainstorm') setBrainstorm((prev)=>[...prev,answer]);
      })
      .subscribe();
  ...
  const discCounts = responses.reduce((acc,r)=>({...acc,[r.primary_type]:(acc[r.primary_type]||0)+1}),{D:0,I:0,S:0,C:0});
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-6 bg-hmh-black">
        ...
        <div className="w-full max-w-xs space-y-4">
          ...
          <button onClick={handleUnlock} className="w-full py-4 rounded-lg font-bold bg-hmh-gold text-hmh-black">Enter</button>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-hmh-black px-4 py-6">
          ...
          {POLL_QUESTIONS.map((q)=>{
            ...
            return (
              <div key={q.id} className="bg-hmh-gray rounded-xl p-5">
                ...
                {qTotal===0?<p className="text-hmh-cream-dim text-sm">No responses yet</p>:(
                      ...
                      return (
                        <div key={opt}>
                          ...
                          <div className="w-full h-3 bg-hmh-gray-light rounded-full overflow-hidden">
                            <div className="h-full bg-hmh-gold rounded-full transition-all duration-700" style={{width:`${pct}%`}} />
                          </div>
                        </div>
                      );
                    })}
      ...
      {tab==='brainstorm' && (
        <div>
          ...
          {brainstorm.length===0?<p className="text-hmh-cream-dim text-sm text-center mt-10">Waiting for responses...</p>:(
            <>
  ...
  return (
    <svg viewBox="0 0 400 400" className="w-full max-w-sm mx-auto block">
      ...
      {responses.map((resp,i)=>{
        const pos=getDotPosition({d:resp.d_score,i:resp.i_score,s:resp.s_score,c:resp.c_score},cx,cy,r);
...
function WordCloud({ texts }) {
  ...
  const colors=['#C9A23E','#F5EDD6','#B91C1C','#4A7FD4'];
  return (
    <div className="bg-hmh-gray rounded-xl p-6 flex flex-wrap gap-3 justify-center items-center min-h-40">
      {sorted.map(([word,count],i)=>(
        <span key={word} style={{fontSize:`${14+Math.round((count/max)*28)}px`,color:colors[i%colors.length],lineHeight:1.3,transition:'all 0.5s'}}>
          {word}
        </span>
      ))}
    </div>
...
export default function DashboardPage() {
  return (
    <Suspense fallback={<main className="min-h-screen flex items-center justify-center bg-hmh-black"><p className="text-hmh-cream-dim">Loading...</p></main>}>
      <Dashboard />
    </Suspense>
  );
}
