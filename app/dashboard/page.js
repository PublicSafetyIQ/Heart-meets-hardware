...
function DiscWheel({ data }) {
  ...
  const getDotPosition = (type, index) => {
    const quadrants = { D: { cx: 65, cy: 30 }, I: { cx: 70, cy: 68 }, S: { cx: 32, cy: 70 }, C: { cx: 28, cy: 30 } };
    const base = quadrants[type] || { cx: 50, cy: 50 };
    const angle = (index * 137.5) % 360;
    const radius = (index % 3) * 4;
        ...
        <>
          <div className="flex flex-wrap gap-2 justify-center mb-8 p-4 bg-hmh-gray rounded-xl">
            {words.map(([word, freq]) => {
              ...
              return (
                <span key={word} style={{fontSize:`${size}px`, opacity, color:'#C9A23E', lineHeight:1.4}}>{word}</span>
              );
            })}
          </div>
