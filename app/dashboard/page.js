...
const WORD_COLORS = ['#C9A23E', '#B91C1C', '#4A90D9', '#7ED07E', '#F5EDD6', '#E07B39', '#A78BFA'];
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-full max-w-xs mx-auto mb-6" style={{aspectRatio:'1'}}>
        <svg viewBox="0 0 100 100" className="w-full h-full">
          <path d="M50,50 L50,2 A48,48 0 0,1 98,50 Z" fill="#B91C1C" opacity="0.2"/>
          <path d="M50,50 L98,50 A48,48 0 0,1 50,98 Z" fill="#C9A23E" opacity="0.2"/>
          <path d="M50,50 L50,98 A48,48 0 0,1 2,50 Z" fill="#1E3A8A" opacity="0.25"/>
          <path d="M50,50 L2,50 A48,48 0 0,1 50,2 Z" fill="#F5EDD6" opacity="0.1"/>
          ...
          {/* Labels centered INSIDE each quadrant */}
          <text x="72" y="30" textAnchor="middle" dominantBaseline="middle" fill="#B91C1C" fontSize="10" fontWeight="bold">D</text>
          <text x="72" y="70" textAnchor="middle" dominantBaseline="middle" fill="#C9A23E" fontSize="10" fontWeight="bold">I</text>
          <text x="28" y="70" textAnchor="middle" dominantBaseline="middle" fill="#4A7DD9" fontSize="10" fontWeight="bold">S</text>
          <text x="28" y="30" textAnchor="middle" dominantBaseline="middle" fill="#ccc" fontSize="10" fontWeight="bold">C</text>
          {data.map((r, i) => {
            ...
            return (
              <circle key={i} cx={pos.x} cy={pos.y} r="2.8"
                fill={DISC_COLORS[type] || '#888'} opacity="0.9" stroke="#000" strokeWidth="0.4"/>
            );
          })}
        </svg>
      </div>
  ...
  return (
    <div className="space-y-8">
      {POLL_QUESTIONS.map(q => {
        ...
        return (
          <div key={q.id}>
            <p className="text-hmh-cream text-sm font-semibold mb-1">{q.label}</p>
                ...
                return (
                  <div key={opt}>
                    ...
                    <div className="h-6 bg-hmh-gray rounded overflow-hidden">
                      <div className="h-full bg-hmh-gold rounded transition-all duration-700" style={{width:`${pct}%`}}/>
                    </div>
                  </div>
                );
              })}
