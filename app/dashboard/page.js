...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    const leadsSub = supabase.channel('leads-live')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'leads' }, payload => {
        setLeadsData(prev => [...prev, payload.new]);
      }).subscribe();
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{background:'#0a0a0a'}}>
      ...
      <div className="flex-1 px-6 pb-10">
        {tab === 'home'      && <HomeView leadsData={leadsData} discCount={discData.length} pollCount={pollData.length} brainstormCount={brainstormData.length} />}
...
function HomeView({ leadsData, discCount, pollCount, brainstormCount }) {
...
function PieChart({ slices, size = 130 }) {
  ...
  slices.forEach(({ value, color }, i) => {
    ...
    if (pct >= 8) {
      ...
      labels.push(<text key={`l${i}`} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle" fill="#fff" fontSize="10" fontWeight="bold">{pct}%</text>);
    }
    angle += slice;
  });
      ...
      {POLL_QUESTIONS.map(q => {
        ...
        return (
          <div key={q.id} className="bg-hmh-gray rounded-xl p-5">
            <p className="text-hmh-cream font-semibold text-base mb-4">{q.label}</p>
                  ...
                  return (
                    <div key={o}>
                      <div className="flex justify-between text-xs mb-1">
                        <span style={{color: OPTION_COLORS[o] || '#ccc'}}>{o}</span>
                        <span className="text-hmh-cream-dim">{count} ({pct}%)</span>
                      </div>
                      <div className="w-full bg-hmh-gray-light rounded-full h-1.5">
                        <div className="h-1.5 rounded-full transition-all" style={{width:`${pct}%`, background: OPTION_COLORS[o] || '#888'}}/>
                      </div>
                    </div>
                  );
                })}
                <p className="text-hmh-cream-dim text-xs pt-1">{total} vote{total !== 1 ? 's' : ''}</p>
              </div>
            </div>
          </div>
        );
